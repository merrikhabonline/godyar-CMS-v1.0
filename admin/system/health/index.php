...
<style>
:root{
    /* نترك هامش بسيط فقط قبل السايدبار */
    --gdy-shell-max: min(1040px, 100vw - 340px);
}
...
/* حاوية المحتوى:
   - مثبتة باتجاه السايدبار (يمين الشاشة في واجهتك)
   - لا تتجاوز (عرض الشاشة - عرض السايدبار) */
.gdy-layout-wrap{
    width:100%;
    max-width:var(--gdy-shell-max);
    margin-left:auto;   /* نلصق المحتوى ناحية السايدبار */
    margin-right:0;
}

/* تقليل الحشو العمودي العام */
...
/* استجابة للجوال: نرجع لعرض كامل بدون حساب السايدبار */
      ...
      <div class="card-body gdy-health-summary">
        <div>
          ...
          <div class="mt-2 gdy-health-meter">
            <div class="gdy-health-meter-fill" style="width: <?= max(10, min(100, (int)$score)) ?>%;"></div>
          </div>
        </div>
        <div class="text-end">
    ...
    <div class="row g-3 mb-3">
      ...
      <div class="col-md-4">
        <div class="card glass-card border-0 shadow-sm h-100">
          <div class="card-body">
            <h5 class="card-title text-white">
              PHP <svg class="gdy-icon me-1" aria-hidden="true" focusable="false"><use href="/assets/icons/gdy-icons.svg#dot"></use></svg>
            </h5>
            <table class="table table-sm table-borderless table-health mb-1">
              <tr>
            ...
            <small class="text-muted">
              الوقت الحالي: <?= h($checks['now']) ?> (<?= h($checks['timezone']) ?>)
            </small>
          </div>
        </div>
      </div>
