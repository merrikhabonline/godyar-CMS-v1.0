...
// =====================
$requestedSlug   = isset($_GET['election']) ? trim((string)$_GET['election']) : '';
...
// مسار المجلد الحالي في الـ URL (مثلاً /godyar)
...
try {
    ...
    foreach ($regions as $rid => &$region) {
        ...
        $code = trim((string)($region['map_code'] ?? ''));
        if ($code !== '') {
            // رابط صفحة تفاصيل الولاية داخل مجلد godyar
...
document.addEventListener('DOMContentLoaded', function () {
  // ==========================
  // 1) Tooltip + الضغط على خريطة السودان
  // ==========================
  ...
  const mapEl      = document.getElementById('gdy-election-map');
  const tooltipEl  = document.getElementById('gdy-el-map-tooltip');

  if (mapEl && tooltipEl && regionsData) {
    Object.keys(regionsData).forEach(function (code) {
      const regionPath = mapEl.querySelector('#' + CSS.escape(code));
      if (!regionPath) return;

      const info   = regionsData[code] || {};
      const infoUrl = info.detail_url || null;

      // تطبيق لون الحزب المتصدر على الخريطة
      if (info.leading_color) {
        regionPath.style.fill = info.leading_color;
      }
      ...
      regionPath.addEventListener('mouseenter', function (ev) {
        const name   = info.name || code;
        const party  = info.leading_party || 'لا بيانات';
        const seatsW = info.seats_won || 0;
        const seatsL = info.seats_leading || 0;
        const total  = info.total_seats || 0;
        const pct    = (info.vote_percent !== null && info.vote_percent !== undefined)
                         ? Number(info.vote_percent).toFixed(2) + '%' : null;

        let bodyHtml = '';
        bodyHtml += '<div>الحزب المتصدر: <strong>' + party + '</strong></div>';
        if (total) {
          bodyHtml += '<div>المقاعد / الدوائر المحسومة: <strong>' + seatsW + '</strong> من ' + total + '</div>';
        }
        if (seatsL) {
          bodyHtml += '<div>المقاعد / الدوائر المتقدمة: <strong>' + seatsL + '</strong></div>';
        }
        if (pct) {
          bodyHtml += '<div>نسبة الأصوات في آخر جولة: <strong>' + pct + '</strong></div>';
        }
        if (infoUrl) {
          bodyHtml += '<div class="mt-1 small" style="opacity:0.8;">انقر لعرض تفاصيل الولاية: السكان، أهم المدن، الأحزاب، الدوائر…</div>';
        }
      ...
      regionPath.addEventListener('mousemove', function (ev) {
        positionTooltip(ev);
      });
      ...
      // الضغط يفتح صفحة تفاصيل الولاية
      if (infoUrl) {
    ...
    function positionTooltip(ev) {
      const wrapper = mapEl.getBoundingClientRect();
      const x = ev.clientX - wrapper.left;
      const y = ev.clientY - wrapper.top;
